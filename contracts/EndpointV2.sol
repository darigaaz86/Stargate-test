...
import { LzTransfer } from "./libs/LzTransfer.sol";
...
contract EndpointV2 is ILayerZeroEndpointV2, MessagingChannel, MessageLibManager, MessagingComposer, MessagingContext {
    ...
    function recoverToken(address _token, address _to, uint256 _amount) external onlyOwner {
        LzTransfer.nativeOrToken(_token, _to, _amount);
    }
    ...
    function _payToken(
        ...
        if (_required > 0) {
            LzTransfer.token(_token, _receiver, _required);
        }
        if (_required < _supplied) {
            unchecked {
                // refund the excess
                LzTransfer.token(_token, _refundAddress, _supplied - _required);
            }
        }
    }
    ...
    function _payNative(
        ...
        if (_required > 0) {
            LzTransfer.native(_receiver, _required);
        }
        if (_required < _supplied) {
            unchecked {
                // refund the excess
                LzTransfer.native(_refundAddress, _supplied - _required);
            }
        }
    }
