...
abstract contract StargateBase is Transfer, IStargate, ITokenMessagingHandler, ICreditMessagingHandler {
    ...
    event InflowAndCharge(bool isTaxi, uint64 amountInSD, uint64 amountOutSD);
    event FeeCharged(uint64 amountOutSD);
    ...
    function _inflowAndCharge(
        SendParam calldata _sendParam
    ) public returns (bool isTaxi, uint64 amountInSD, uint64 amountOutSD) {
        ...
        emit InflowAndCharge(isTaxi, amountInSD, amountOutSD);
    }
    ...
    function _chargeFee(FeeParams memory _feeParams, uint64 _minAmountOutSD) public returns (uint64 amountOutSD) {
    ...
    function _rideBus(
        ...
        uint64 _amountSD,
        address _refundAddress
    ) public virtual returns (MessagingReceipt memory receipt, Ticket memory ticket) {
        console.log("Starting _rideBus function");
        ...
        console.log("busFare:", busFare, "providedFare:", providedFare);
    ...
    function _sd2ld(uint64 _amountSD) public view returns (uint256 amountLD) {
        unchecked {
    ...
    function _ld2sd(uint256 _amountLD) public view returns (uint64 amountSD) {
        unchecked {
    ...
    function _isTaxiMode(bytes calldata _oftCmd) public pure returns (bool) {
    ...
    function _inflow(address _from, uint256 _amountLD) public virtual returns (uint64 amountSD);
    ...
    function _buildFeeParams(
        ...
        uint64 _amountInSD,
        bool _isTaxi
    ) public view virtual returns (FeeParams memory);
