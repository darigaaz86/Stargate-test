...
contract StargatePoolNativeV2 is StargatePool {
    ...
    function _inflow(address /*_from*/, uint256 _amountLD) public view override returns (uint64 amountSD) {
        amountSD = _ld2sd(_amountLD);
    }

    function _postOutflow(uint64) internal pure override {
        console.log("start _postOutflow");
    }

    function _outflow(address, uint256) internal pure override returns (bool success) {
    ...
    function _assertMessagingFee(
        ...
        uint256 _amountInLD
    ) public payable override returns (MessagingFee memory) {
