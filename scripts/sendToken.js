...
// after run the deployStargate, put the op sepolia pool address to the CONTRACT_ADDRESS
async function sendtoken() {
    // Contract and network setup
    const YOUR_POOL_ADDRESS = "0xAc8e7Bb4f1bE79B0e96F3CA182D94743Cd0c37A1"; // Replace with actual contract address
    const recipientAddress = "0x3e6AFD653D62d7D797C7c2a0a427a17a6457C56D";
    ...
    const contract = await ethers.getContractAt(contractABI, YOUR_POOL_ADDRESS);


    // Convert recipient address to bytes32
    const toBytes32 = ethers.zeroPadValue(recipientAddress, 32); // Convert address to bytes32

    // Format input parameters as arrays
    const _sendParam = [
        40161,  // dstEid
        toBytes32,  // to (recipient address in bytes32)
        10000000000000,  // amountLD
        9000000000000,  // minAmountLD
        "0x",  // extraOptions
        "0x",  // composeMsg
        "0x1a"  // oftCmd
    ];
    ...
    const _fee = [
        100000000000000,  // nativeFee
        0  // lzTokenFee
    ];

    const _refundAddress = recipientAddress; // Refund Address

    // **Set the exact transaction value**
    const txOptions = {
    ...
    const tx = await contract.send(_sendParam, _fee, _refundAddress, txOptions);
